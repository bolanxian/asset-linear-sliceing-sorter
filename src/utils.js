    ...
    async*entries(onprogress){
      ...
      for(var name of pngs.keys()){
        ...
        if(tag(img)==='File'&&tag(mesh)==='File'){
          ...
          yield [name,Mesh.from(await mesh,await img)]
        }
      }
      if(!Mesh.PantingExtracter)return
      for(var name of bundles?bundles.keys():[]){
        var file=bundles.get(name)
        if(tag(file)==='FileSystemFileHandle'){
          file=await file.getFile()
        }
        yield [name,await Mesh.fromBundle(await file.arrayBuffer(),(stat)=>{
          onprogress({
  ...
  class MeshFsHandleLoader{
    ...
    async get(name){
      ...
      try{
        ...
        return Mesh.from(await mesh,await img)
      }catch(err){
